...
def get_one_day_data(year, month, day, data_type):
    ...
    folder_name = 'downloaded_csvs'
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)

    output_filename = 'TDCS_' + data_type + '_' + timestep_str_d + '.csv'
    target_df.to_csv(os.path.join(folder_name, output_filename), index=False)
...
def data_c(df):
    ...
    df_5.to_csv(os.path.join(folder_name, 'TDCS_M03A_20240429_5.csv'), index=False)
